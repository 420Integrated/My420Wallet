...
import FourtwentyscanProvider from './providers/fourtwentyscan-provider';
...
class MEWProvider {
  constructor(host, options, store, eventHub) {
    if (store.state.wallet && store.state.wallet.identifier == WEB3_WALLET) {
      ...
      if (host.includes('fourtwentyscan')) {
        return new FourtwentyscanProvider(host, options, store, eventHub);
